 ...
 * which require constructors and destructors to be called.
 */
template <class T, class A = BasicAllocator>
class BasicArray {
 ...
 public:
   ...
   */  
  ~BasicArray() {if (data) A::free(data);}
  ...
  /**
   * Move the contents of another array into this one.
   * @param a The other array will be empty after this call.
   */
  void adopt(BasicArray<T, A> &a) {
    reset();
    size = a.size;
    capacity = a.capacity;
    data = a.adopt();
  ...
  void increase(const unsigned int minCapacity,
    ...
    void *newMem = A::realloc(data, capacity * sizeof(T));
  ...
  void shrink() {
    if (capacity > size) {
      data = (T *)A::realloc(data, size * sizeof(T));
      capacity = size;
    }
  }
  ...
  T &get(const unsigned int i) const {
    ASSERT_OR_THROW("BasicArray index out of range!", i < size);
    return data[i];
  }
