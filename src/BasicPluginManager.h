...
private:
  ...
  typedef typename std::map<std::string, lt_dlhandle> libraryHandles_t;
...
public:
  ...
  BasicPluginManager(bool preloadDeps = false) :
    ...
    if(result != 0) {
      std::cout << "Error initializing ltdl" << std::endl;
    }
  }
  ...
  void loadLibrary(const std::string filename) {
    ...
    lt_dlhandle handle = lt_dlopen(filename.c_str()); // , RTLD_LAZY | RTLD_GLOBAL);
    const char *error;
    if ((handle == NULL) && (error = lt_dlerror())) {
      std::cout << "Error Loading " << filename << " " << error << endl;
